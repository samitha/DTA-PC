...
public class IntertemporalSplitRatios {
	...
	public void addNonCompliantSplitRatios(DiscretizedGraph g,
		...
		for (int j = 0; j < non_compliant_split_ratios.length; j++) {
			...
			IntertemporalJunctionSplitRatios ijsr = get(g.nodeToJunction(
					node_id).getUniqueId());
			...
			for (int k = 0; k < tmp.length; k++) {
				...
				if (orig != null) {
					ijsr.addNonCompliantSplitRatio(tmp[k].k,
							orig.getEntries()[0].getUniqueId(), g
									.firstCellofLink(tmp[k].out_id)
									.getUniqueId(), tmp[k].c, tmp[k].beta);
				} else
					ijsr.addNonCompliantSplitRatio(tmp[k].k,
							g.lastCellofLink(tmp[k].in_id).getUniqueId(), g
									.firstCellofLink(tmp[k].out_id)
									.getUniqueId(), tmp[k].c, tmp[k].beta);
			}
		}
	 ...
	 *          road to an other multiplied by the number of time steps. It can
	 *          be improved since a lot of the split-ratios can be zero.
	 * @param junctions
	...
	public boolean check_data_integrity(Junction[] junctions) {
		...
		for (int j_id = 0; j_id < junctions.length; j_id++) {
			...
			/*
			 * We check the integrity of the non-compliant split ratios. The
			 * integrity of the compliant split-ratios is verified by
			 * construction since it is generated from the associated path.
			 */
			double total = 0;
			HashMapTripletDouble[] non_compliant_split_ratios = get(j_id).non_compliant_split_ratios;

			for (int k = 0; k < non_compliant_split_ratios.length; k++) {
				for (int i = 0; i < in.length; i++) {
					for (int j = 0; j < out.length; j++) {
						Double result = non_compliant_split_ratios[k]
								.get(new Triplet(in[i].getUniqueId(), out[j]
										.getUniqueId(), 0));
						if (result != null) {
				...
				if (!Numerical.equals(total, 1.0, 0.001)) {
					System.out
							.println("[Warning] The sum of the non-compliant "
									+ "split-ratios at junction " + j_id
									+ " is not 1" + " (" + total + ")");
					is_valid = false;
				}
			}
